...
/** Recent follows, straight from Twitch. Real events arrive in Phase 7. */
export function RecentFollowers({ state }: { state: ChannelState }) {
  return (
    <Panel>
      ...
      {state.status !== 'ok' ? (
        <EmptyState
          title="No events yet"
          description="Connect Twitch to see recent follows here. Live events — subs, raids and cheers — arrive in Phase 7."
        />
      ) : state.recentFollowers.length === 0 ? (
        <EmptyState
          title={state.canSeeFollowers ? 'No recent follows' : 'Follower list unavailable'}
          description={
            state.canSeeFollowers
              ? 'New followers will appear here as they arrive.'
              : 'Twitch only shares the follower list with the broadcaster. Reconnect to grant that permission.'
          }
        />
      ) : (
        <ul>
          {state.recentFollowers.map((follower) => (
            <li
              key={`${follower.userName}-${follower.followedAt}`}
              className="flex items-center justify-between gap-3 border-b border-line px-5 py-3 last:border-b-0"
            >
              <span className="text-sm text-ink">{follower.userName}</span>
